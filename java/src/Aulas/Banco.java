...
public class Banco {// inicio
	...
	public static void contaPoupanca() {
		...
		for (int giro = 1; giro <= 10; giro++) {
			...
			opcaoDC = tec.next().toUpperCase().charAt(0);// charAt cpega o que digitaa na primeira posicao e transforma
															// em caracter toupper transforma em letra grande
			if (opcaoDC == 'D') {
				...
				for (int i = 1; i <= 10; i++) {
					...
					opcaoDC = tec.next().toUpperCase().charAt(0);
					{
						...
						if (saldo > 0) {
							...
							while (debito > saldo) {
								System.out.println(
										"Você está utilizando o limite estudantil. Digite o valor a ser debitado: ");
								debito = tec.nextDouble();
							}
							tec.nextLine();
							System.out.println("Digite a descrição do débito: ");
